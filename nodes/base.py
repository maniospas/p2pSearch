...
class Node:
    ...
    def receive_embedding(self, neighbor, neighbor_embedding):
        ...
        if neighbor in self.neighbors:
            self.embedding += (neighbor_embedding - self.neighbors[neighbor]) / N**0.5 * 0.9
        else:
            self.embedding = ((self.embedding - 0.1 * self.personalization) * N**0.5
                              + neighbor_embedding * 0.9) / (N+1)**0.5 + 0.1 * self.personalization
...
class DocNode(ABC):
    ...
    def receive(self, queries, from_node, kill_seen=False):
        for query in queries:
            if query.name not in self.seen_from:
                query.check_now(self.docs)
            elif query.name in self.seen_from and kill_seen:
