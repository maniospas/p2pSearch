...
class Antique:
    ...
    def __init__(self, mode="train"):
        ...
        if mode == "train":
            dset = ir_datasets.load("antique/train/split200-train")
        elif mode == "validate":
            dset = ir_datasets.load("antique/train/split200-valid")
        elif mode == "test":
            dset = ir_datasets.load("antique/train/test")
        else:
            raise Exception(f"mode must be train, validate, or test, {mode} given")
    ...
    def stream_documents(self, as_str=False):
        for doc in self.dset.docs_iter():
            yield doc.text if as_str else TextRecord(doc.doc_id, doc.text)
    ...
    def stream_qrels(self, as_str=True, binary=True):
        for qrel in self.dset.qrels_iter():
            ...
            rel = qrel.relevance
            if binary:
                rel = 1 if qrel.relevance >= 3 else 0
            yield (query, doc, rel)
